...
while True:
    ...
    mask_trajectory = np.broadcast_to(obs[2], shape=(3, r_obs, r_obs)).transpose(1, 2, 0)
    ...
    img = np.where(np.logical_not(mask_trajectory), np.array([0, 0, 0], dtype=np.uint8), img)
