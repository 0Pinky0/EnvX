...
class FunctionalJaxEnv(gym.Env):
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        frontier_t = jax_to_numpy(self.state.map_frontier).astype(np.uint8)
        self.retval_t, _, _, _ = cv2.connectedComponentsWithStats(frontier_t, connectivity=8)
    ...
    def step(self, action: ActType):
        ...
        frontier_tp1 = jax_to_numpy(next_state.map_frontier).astype(np.uint8)
        retval_tp1, _, _, _ = cv2.connectedComponentsWithStats(frontier_tp1, connectivity=8)
        retval_delta = retval_tp1 - self.retval_t
